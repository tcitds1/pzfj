...
var maxPathSum = function(root) {
  ...
  function gain_max(root) {
    if (root === null) {
      return 0
    }
    let left_max_val = Math.max(gain_max(root.left), 0)
    let right_max_val = Math.max(gain_max(root.right), 0)
    let sum = root.val + left_max_val + right_max_val
    max = Math.max(max, sum)
    return (root.val + Math.max(left_max_val, right_max_val))
  }
  gain_max(root)
  return max
};
